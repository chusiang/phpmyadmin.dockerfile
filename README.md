# phpMyAdmin Docker image (443 port)
...
All following examples will bring you phpMyAdmin on `http://localhost:443`
...
```
docker run --name myadmin -d --link mysql_db_server:db -p 443:443 phpmyadmin/phpmyadmin
```
...
```
docker run --name myadmin -d -e PMA_HOST=dbhost -p 443:443 phpmyadmin/phpmyadmin
```

```
docker run --name myadmin -d -e PMA_HOSTS=dbhost1,dbhost2 -p 443:443 phpmyadmin/phpmyadmin
```
...
```
docker run --name myadmin -d --link mysql_db_server:db -p 443:443 -e PMA_ARBITRARY=1 phpmyadmin/phpmyadmin
```
